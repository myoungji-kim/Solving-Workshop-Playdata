...
public class EmpDAO {
	...
	public int insert(SqlSession session, EmpDTO dto) {
		int num = session.insert("com.config.EmpMapper.insert", dto);
		return num;
	}  // end insert
	
	// 사원번호 있는지 체크
	public List<Integer> selectAllEmpno(SqlSession session){
		return session.selectList("com.config.EmpMapper.selectAllEmpno");
	}
